...
> = {
  ko: {
    ...
    descriptionLines: ['영화 한 편이 하나의 우주가 돼요.', '감독과 장르로 가까워지기도 하고,', '직접 손으로 배치할 수도 있어요.'],
    howToTitle: '어떻게 쓰나요',
    steps: [
      { n: '01', t: '기록한다', d: '영화를 보고 평점과 메모를 남겨요.' },
      { n: '02', t: '우주가 된다', d: '감독, 장르로 자연스럽게 모이거나, 직접 배치하고 이름 붙일 수 있어요.' },
      { n: '03', t: '탐색한다', d: '확대하고 영화를 눌러 관계를 발견해요.' },
      { n: '04', t: '공유한다', d: '우주 전체나 영화 한 편을 링크로 보여줘요.' },
    ],
    philosophyLines: ['영화를 볼수록 내 우주가 넓어져요.', '나를 채운 영화들이 모여,', '광활하고 끝없는 하나의 세계가 돼요.'],
    tryTitle: '지금 둘러보기',
    trySubtitle: '80편으로 채운 데모 우주를 로그인 없이 체험할 수 있어요.',
    enterAriaLabel: '데모 우주로 들어가기',
    pressToEnterLabel: '눌러서 들어가기',
    faqTitle: '자주 묻는 질문',
    faq: [
      { q: '로그인 안 해도 볼 수 있어요?', a: '네. 데모 우주는 누구나 로그인 없이 둘러볼 수 있어요. 본 영화로 채운 진짜 우주를 가지려면 로그인이 필요해요.' },
      { q: '무료예요?', a: '네, 완전히 무료예요.' },
      { q: '내 정보가 다른 사람에게 보여요?', a: '아니요. 직접 공유 링크를 켜지 않는 한 아무도 못 봐요.' },
    ],
    finalCtaSentence: '지금 우주를 시작해보세요.',
  ...
  return (
    <div ref={rootRef} className="relative flex min-h-full w-full flex-col items-center bg-black">
      ...
      <footer className="flex w-full flex-col items-center gap-4 px-6 py-16">
        ...
        {/* TMDB API 이용약관이 요구하는 출처 고지 — 영문 문구를 그대로 노출해야
            하는 의무 조건이라 번역하지 않는다. */}
        <p className="text-[8px] font-light tracking-[var(--tk-10)] text-white/10">
