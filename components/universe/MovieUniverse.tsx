...
// "가까운 별일수록"이라고만 쓰면 옆에 붙어있는 다른 포스터끼리 관계가 있다는
// 뜻으로 읽힐 수 있다 — 실제로는 오직 중심 영화와의 거리만 관계를 나타내고,
// 각도(어느 방향에 있는지)는 그냥 고정 슬롯이라 이웃한 포스터끼리는 아무
// 관계가 없을 수도 있다. "중심과"를 명시해서 그 오해를 없앤다.
// 세 번째 문장도 예전엔 "별을 눌러 다른 영화로 이동해"였는데, 지금은 클릭이
// 열람(peek)만 열 뿐 우주를 재배치하지 않는다 — 실제로 이동하려면 열람 패널
// 안의 "이 영화를 중심으로"를 한 번 더 눌러야 한다. 클릭의 실제 동작(자세히
// 보기)에 맞게 고쳤다.
const IDLE_HINTS = ['확대해서 둘러봐', '중심 영화와 가까울수록 관계가 깊어', '포스터를 눌러 자세히 봐'] as const
