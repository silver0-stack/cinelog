...
// 사례). 각도 자체를 사용자가 고른 기준(장르/감독/시대)으로 묶어서, 같은
// 기준을 공유하는 영화들이 반드시 같은 방향(부채꼴 섹터)에 모이게 한다 —
// "박찬욱 영화 3편"이 실제로 화면에서도 뭉쳐 보이는 걸 물리 시뮬레이션 없이
// 만든다. 반지름(관계 강도)은 그대로 두고 각도 배정 방식만 바꾼 것이라
// MIN_RADIUS/MAX_RADIUS/relatedMovies 쪽 로직은 전혀 안 건드린다.
export type GroupMode = 'genre' | 'director' | 'era'

function clusterKey(movie: Movie, mode: GroupMode): string {
  if (mode === 'genre') return movie.genres[0] || '기타'
  if (mode === 'director') return movie.director || '기타'
  return movie.era || '기타'
}

/**
 * movie.id → 각도(라디안) 맵을 만든다. 같은 클러스터 키(장르/감독/시대)를
 * 가진 영화들은 자기 섹터 안에서만 각도를 배정받는다 — 섹터 경계를 넘어가는
 * 일이 없어서, 같은 기준을 공유하는 영화들은 항상 시각적으로 뭉쳐 보인다.
 *
 ...
 * 더 넓게 펼쳐질 공간을 준다.
 */
export function clusterAngles(movies: Movie[], mode: GroupMode): Map<string, number> {
  const groups = new Map<string, Movie[]>()
  for (const movie of movies) {
    const key = clusterKey(movie, mode)
