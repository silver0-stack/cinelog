...
export const UNIVERSE_QA: Record<Locale, QA[]> = {
  ko: [
    {
      q: '영화들이 왜 이 자리에 있어요?',
      a: '기본적으로는 우주 전체에서 자기와 가장 강하게 이어진 한 편과의 관계로 자리가 정해져요(같은 감독, 겹치는 장르·테마일수록 안쪽으로, 같은 장르는 같은 방향으로). 별을 직접 드래그해서 옮기면 그때부터는 그 자리가 그대로 저장되고, 다시는 자동으로 안 움직여요 — 데이터로는 안 잡히는 나만의 연결을 표현하는 거예요.',
    },
    {
      q: '제목 밑에 보이는 평점/메모는 뭐예요?',
      a: '그 영화에 남긴 감상 미리보기예요. 별점 옆에 "2회"처럼 숫자가 있으면 그 영화를 다시 본 횟수예요. 눌러서 전체를 볼 수 있어요.',
    },
    {
      q: '눌러보면?',
      a: '카메라가 그 포스터로 확대해서 다가가고, 옆(좁은 화면에서는 아래)에 평점·메모·감상 이력이 펼쳐져요. 동시에 그 영화와 관계 깊은 별들만 밝아지고 나머지는 어두워져요 — 재배치 없이 관계를 보여주는 거예요. 포스터를 다시 누르거나 빈 공간을 누르면 원래대로 돌아가요.',
    },
    {
      q: '확대·축소는 어떻게 해요?',
      a: '데스크톱은 Ctrl + 마우스 휠, 모바일은 두 손가락으로 핀치예요. 빈 공간을 드래그하면 화면이 움직여요. 많이 축소하면 포스터가 그 영화 색의 흐린 빛으로 접혀 보여요. 다시 확대하면 원래대로 돌아와요.',
    },
  ],
  en: [
    {
...
export const DEMO_QA: Record<Locale, QA[]> = {
  ko: [
    { q: '이 우주는 진짜예요?', a: '내가 직접 고른 80편이에요. 로그인하면 이 자리에 실제로 본 영화들로 채운 진짜 우주가 생겨요.' },
    { q: '평점이나 메모를 남길 수 있어요?', a: '네, 데모 80편 어디에나 자유롭게 남겨볼 수 있어요. 저장은 안 되고 새로고침하면 사라져요. 로그인하면 진짜로 쌓여요.' },
  ],
  en: [
...
export const ARCHIVE_QA: Record<Locale, QA[]> = {
  ko: [
    { q: '"다시 본 감상 남기기"는 뭐예요?', a: '이 영화를 또 봤을 때 새 감상을 남기는 거예요. 이전 감상을 덮어쓰지 않고 그대로 쌓여요.' },
    { q: '우주 공유랑 영화 카드 공유는 뭐가 달라요?', a: '우주 공유는 내 아카이브 전체를 보여주는 링크고, 영화 카드 공유는 그 영화 한 편만 보여주는 링크예요.' },
    {
      q: '히스토리는 뭐예요?',
      a: '내 우주가 시간이 지나며 어떻게 자라났는지 다시 보는 기능이에요. "탐색" 패널 안의 히스토리 보기를 누르면 화면 아래 타임라인이 뜨고, 드래그하면 그 시점까지 기록한 영화만 빛나 보여요.',
    },
  ],
  en: [
...
export const SHARED_QA: Record<Locale, QA[]> = {
  ko: [
    { q: '이 우주는 뭐예요?', a: '이 링크를 보낸 사람이 실제로 기록한 영화들이에요. 로그인하면 똑같은 방식으로 우주를 만들 수 있어요(화면 구석 "나도 기록하기").' },
  ],
  en: [
    {
